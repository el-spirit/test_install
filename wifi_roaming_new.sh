...
# ====================== ПАКЕТЫ (жёсткое исправление) ======================
echo "[*] Установка пакетов..."

if command -v apk >/dev/null 2>&1; then
    apk update
    
    echo "[*] Удаляем все версии wpad..."
    apk remove wpad wpad-basic wpad-basic-mbedtls wpad-basic-openssl wpad-mini 2>/dev/null || true
    
    echo "[*] Устанавливаем полную версию..."
    if ! apk add --no-cache wpad-openssl; then
        apk add --no-cache wpad
    fi
else
    opkg update
